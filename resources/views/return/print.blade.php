...
<head>
    ...
    <title>Nota Pengembalian - {{ $return->code }}</title>
    <style>
...
<body onload="window.print()">
    <div class="no-print">
        <a href="{{ route('return.show', $return) }}" class="btn-back">KEMBALI</a>
    ...
    <div class="nota-info text-center">
        <div class="bold" style="font-size: 16px; border: 1px solid #000; display: inline-block; padding: 2px 10px; margin-bottom: 3mm;">STRUK PENGEMBALIAN</div>
        <div class="bold">NOMOR: {{ $return->code }}</div>
        <div>TANGGAL: {{ $return->created_at->format('d/m/Y H:i') }}</div>
        <div class="divider"></div>
        <div class="bold">PROJECT:</div>
        <div class="uppercase">{{ $return->borrowTransaction?->project?->name ?? 'N/A' }}</div>
        <div class="bold">PETUGAS:</div>
        <div class="uppercase">{{ $return->receiver?->name ?? 'N/A' }}</div>
    </div>
        ...
        <tbody>
            <tr>
                <td class="text-center">
                    <span class="item-name uppercase">{{ $return->borrowItem?->inventory?->name ?? 'UNKNOWN ASSET' }}</span>
                    <span class="item-qty">
                        {{ $return->quantity_good + $return->quantity_damaged + $return->quantity_lost }} 
                        {{ strtoupper($return->borrowItem?->inventory?->unit ?? 'pcs') }}
                    </span>
                    <span style="font-size: 10px;">ID: {{ $return->borrowItem?->inventory?->code ?? 'N/A' }}</span>
                </td>
            </tr>
        </tbody>
    </table>
    ...
    <div class="nota-info">
        <div class="divider"></div>
        <div class="bold uppercase">RINCIAN KONDISI:</div>
        <div>BAIK: {{ $return->quantity_good }}</div>
        <div>RUSAK: {{ $return->quantity_damaged }}</div>
        <div>HILANG: {{ $return->quantity_lost }}</div>
        
        @if($return->condition_notes)
        <div style="margin-top: 3mm; font-style: italic;">"{{ $return->condition_notes }}"</div>
        @endif
    </div>

    <div class="footer text-center">
        <div class="divider"></div>
        <p class="bold uppercase" style="margin-bottom: 1mm;">PENGEMBALIAN BERHASIL</p>
        <p style="margin: 0; font-size: 10px;">TERIMA KASIH TELAH MENJAGA ASSET</p>
        
        <div class="qr-section">
            <img src="https://api.qrserver.com/v1/create-qr-code/?size=200x200&data={{ urlencode(route('return.show', $return)) }}" class="qr-code">
        ...
        <p class="bold" style="letter-spacing: 2px;">*** WAREHOUSE SYSTEM ***</p>
    </div>
</body>
</html>
